...
def main():
	...
	writeHeader(f)
	writeList(f, num_used_checks, names_of_used, args, external_link, external_name)
	sortLogs(f, contents, num_used_checks, names_of_used, args, external_link, external_name)
...
def writeList(f, num_used_checks, names_of_used, args, external_link, external_name):
	...
	f.write("\t\t<h3 style=\"text-align: center; color: #111; font-family: 'Helvetica Neue', sans-serif; font-weight: bold; letter-spacing: 0.5px; line-height: 1;\">Clang-Tidy Checks</h3>\n")
	...
	f.write("\t\t\t<button id=\"collapse-btn0\" type=\"button\" class=\"btn nohover\" onclick=\"collapseSidebar()\" style=\"outline: none; background-color: lightgray\" title=\"Collapse sidebar\">\n")
	...
	if (args.button):
	    ...
	    f.write("\t\t\t%s <span class=\"glyphicon glyphicon-new-window\"></button></span>\n" % external_name)
